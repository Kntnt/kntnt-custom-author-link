...
abstract class Abstract_Settings {
	...
	public function add_plugin_action_links( $actions ) {
		$settings_link_name = __( 'Settings', 'kntnt-custom-author-link' );
	...
	public function options_page() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'Unauthorized use.', 'kntnt-custom-author-link' ) );
		}
	...
	private function render_settings_page() {
		...
		if ( $unsatisfied_dependencies = Plugin::unsatisfied_dependencies() ) {
			$message = sprintf( _n( 'This plugin must be installed and active: %s', 'These plugins must be installed and active: %s', count( $unsatisfied_dependencies ), 'kntnt-custom-author-link' ), join( ', ', $unsatisfied_dependencies ) );
	...
	private function update_options( $opt ) {
		...
		if ( ! wp_verify_nonce( $_POST['_wpnonce'], $this->ns ) ) {
			wp_die( __( 'Nonce failed.', 'kntnt-custom-author-link' ) );
		}
	...
	private function notify_error( $field ) {
		if ( isset( $field['validate-error-message'] ) ) {
			...
			$message = sprintf( __( '<strong>ERROR:</strong> Invalid data in the field <em>%s</em>.', 'kntnt-custom-author-link' ), $field['label'] );
		}
		else {
			$message = __( '<strong>ERROR:</strong> Please review the settings and try again.', 'kntnt-custom-author-link' );
		}
		$this->notify_admin( $message, 'error' );
	}

	private function notify_success() {
		$message = __( 'Successfully saved settings.', 'kntnt-custom-author-link' );
